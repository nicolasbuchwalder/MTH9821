...
BinomialTree::BinomialTree(double S0, double sigma, double T, std::size_t steps, double r, double q) : S0_(S0), sigma_(sigma), T_(T), steps_(steps), r_(r), q_(q),
    dt_(T / steps),
...
double BinomialTree::r() const { return r_; }
double BinomialTree::q() const { return q_; }
...
TreeResult BinomialTree::TreePricerBS_PathDependent(const std::function<double (double, double)>& payoff) const {
    ...
    // S[1]: Asset price dt before maturity
    std::array<std::deque<double>, 2> S;
    S[0].push_back(S0_ * std::pow(u_, static_cast<double>(curr_step)));
    S[1].push_back(S0_ * std::pow(u_, static_cast<double>(curr_step) - 1.));
